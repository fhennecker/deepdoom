...
engineering technique prevents overfitting.
...
Finally, we export a video every 200 played games, where the agent plays on itw
...
\subsubsection{FOMP} We first allow the agent to play the following actions:
\begin{itemize}
...
therefore, the ennemy never goes out of sight for the agent and the state is
fully observable.

\subsubsection{POMP} We then change the agent's action set to the following:
\begin{itemize}
...
With the action set, the agent is now able to move such that the ennemy is no
